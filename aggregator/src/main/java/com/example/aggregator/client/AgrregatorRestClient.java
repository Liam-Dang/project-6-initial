...
@Component
public class AgrregatorRestClient {

    private RestTemplate restTemplate;

    public AgrregatorRestClient(RestTemplate restTemplate) {
        this.restTemplate = restTemplate;
    }

    public Entry getDefinition(String word) {

        String uri = "http://localhost:9091/getWord/" + word;

        Entry result = this.restTemplate.getForObject(uri, Entry.class);
